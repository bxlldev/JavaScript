...
//     ? console.log(`${dog.owners}'s dog eating too much`)
//     : console.log(`${dog.owners}'s dog eating too little`)
// );
...
const bankDepositSum = accounts
  .flatMap(acc => acc.movements) // create new array + flat them out
  .filter(mov => mov > 0) // filter only positive array
  .reduce((sum, cur) => sum + cur, 0); // accumulate array
...
const numDeposits1000v3 = accounts
  .flatMap(acc => acc.movements)
  .reduce((count, cur) => (cur >= 1000 ? ++count : count), 0);

console.log(numDeposits1000); // 6 elements
console.log(numDeposits1000v2); // 6 elements
console.log(numDeposits1000v3); // 6 elements

// Reference (Prefixed + operator)
let a = 10;
console.log(++a); // ++a => executed before, a++ => executed after
console.log(a); //after executed => a = 11
